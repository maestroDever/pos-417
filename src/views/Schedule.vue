<template>
  <v-flex>
    <v-flex mb-4 text-xs-center>
      <h1 class="display-3">This Weeks Schedule</h1>
    </v-flex>
    ...
    <div>
      ...
      <v-flex text-xs-center mt-2>
        <v-btn large to="/schedule1">
          See Next Weeks Schedule
...
import { db, thisWeeksScheduleDates } from "@/fb";
...
export default {
  ...
  data: () => ({
    ...
    search: "",
    scheduleUpdates:
      "*** Storey working for Brandi on Tuesday from 11AM - 2PM ***",
    headers: [
      {
        text: "Shift",
  ...
  created() {
    this.initialize();
    db.collection("schedule").onSnapshot(res => {
    ...
    db.collection("thisWeeksScheduleDates")
      .get()
      .then(snapshot => {
