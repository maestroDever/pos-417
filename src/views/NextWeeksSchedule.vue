    ...
    <v-card flat color="rgb(255, 0, 0, 0.0)">
      <v-layout mt-3>
        <v-flex text-xs-center mb-3>
          <h1 class="display-3">Schedule Management</h1>
        </v-flex>
      </v-layout>
    </v-card>
    <div>
      <v-toolbar height="95">
        ...
        <v-flex mt-2>
          ...
          >
            ...
            <v-date-picker v-model="date2" @input="menu2 = false" first-day-of-week="1"></v-date-picker>
          </v-menu>
        </v-flex>
      ...
      <v-toolbar flat height="40">
        <v-flex mt-2 mb-4>
          <v-btn small class="orange">
            <v-icon left>chevron_left</v-icon>Transfer
          </v-btn>
        </v-flex>
      ...
      <v-flex text-xs-center mt-2>
        <v-btn large to="thisweeksschedule">
          <v-icon left>arrow_back</v-icon>This Weeks Schedule
        </v-btn>
      </v-flex>
    </div>
...
<script>
import { db, schedule2Collection, nextWeeksScheduleDates } from "@/fb";
...
export default {
  ...
  created() {
    schedule2Collection.get().then(snapshot => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Are you sure you want to delete this shift?")) {
        ...
        schedule2Collection
          ...
          .then(() => {})
          .catch(error => {
            alert("Error occured while trying to delete the shift.");
            console.log(error);
          });
      }
    },
