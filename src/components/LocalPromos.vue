  ...
  <v-container>
    <v-card>
      <v-flex text-xs-center>
        ...
        <v-flex v-for="item in localPromos" :key="item">
...
export default {
  ...
  created() {
    localPromos.get().then(snapshot => {
      snapshot.forEach(doc => {
        this.localPromos.push({
          id: doc.id,
          one: doc.data().one,
          two: doc.data().two,
  ...
  methods: {
    ...
    editItem(item) {
      this.selectedIndex = this.localPromos.indexOf(item);
    ...
    deleteItem(item) {
      const index = this.localPromos.indexOf(item);
      if (confirm("Are you sure you want to delete this item?")) {
        this.localPromos.splice(index, 1);
        localPromos
          .doc(item.id)
          .delete()
          .then(() => {})
    ...
    save() {
      if (this.selectedIndex > -1) {
        Object.assign(this.localPromos[this.selectedIndex], this.selectedItem);
        localPromos
        ...
        this.localPromos.push(this.selectedItem);
        localPromos
          .doc(this.selectedItem.id)
          .set(this.selectedItem)
          .then(docRef => {
