<template>
  <div id="app">
    <div>
      <v-toolbar height="75">
        <v-toolbar-title>Past Orders</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      this.selectedIndex = this.pastOrders.indexOf(item);
    ...
    deleteItem(item) {
      const index = this.pastOrders.indexOf(item);
      if (confirm("Are you sure you want to delete this item?")) {
        this.pastOrders.splice(index, 1);
        pastOrders
          .doc(item.id)
          .delete()
          .then(() => {})
    ...
    save() {
      if (this.selectedIndex > -1) {
        Object.assign(this.pastOrders[this.selectedIndex], this.selectedItem);
        pastOrders
        ...
        this.pastOrders.push(this.selectedItem);
        pastOrders
          .doc(this.selectedItem.id)
          .set(this.selectedItem)
          .then(docRef => {
