  ...
  <v-container>
    <v-card>
      <v-flex text-xs-center>
        <v-dialog dark persistent v-model="dialog" max-width="850px">
        ...
        <v-flex v-for="item in storeDashboard" :key="item">
          <h3 class="highlight">{{ item.one }}</h3>
          <h3>{{ item.two }}</h3>
          <h3>{{ item.three }}</h3>
          <h3 class="highlight">{{ item.four }}</h3>
          <h3>{{ item.five }}</h3>
        </v-flex>
      </v-flex>
    </v-card>
